...
import ${package}.ui.AppUI.Servlet;
...
public class MyNavigator extends Navigator implements ViewCacheContainer
{
    ...
    private static Logger logger = LoggerFactory.getLogger(MyNavigator.class);
    ...
    public MyNavigator(UI ui, NavigationStateManager stateManager, ViewDisplay display)
    {
    ...
    protected void initViews()
    {
        if (views.isEmpty())
        {
            ...
            String[] beansName = getApplicationContext().getBeanDefinitionNames();
            
            // Also looking for parent's beans definition
            if (getApplicationContext().getParent() != null)
            {
                String[] parentBeansName = getApplicationContext().getParent().getBeanDefinitionNames();
            ...
            for (String beanName : beansName)
            {
                ...
                if (beanClass != null && beanClass.isAnnotationPresent(VaadinView.class) && View.class.isAssignableFrom(beanClass))
                {
                    ...
                    ViewCache viewCache = new ViewCache(viewName, beanName, (Class< ? extends View>) beanClass, viewCached);
                    views.add(viewCache);
    ...
    public void addBeanView(String viewName, Class<? extends View> viewClass, boolean cached)
    {
        ...
        String[] beanNames = getApplicationContext().getBeanNamesForType(viewClass);
    ...
    public View getView(String name, String beanName, boolean cached)
    {
        if (cached)
        {
            ...
            View view = (View) getApplicationContext().getBean(beanName);
            viewScoped.put(name, view);

            return view;
        }

        return (View) getApplicationContext().getBean(beanName);
    }

    private ApplicationContext getApplicationContext()
    {
        AppUI.Servlet servlet = (Servlet) VaadinServlet.getCurrent();
        return (ApplicationContext) servlet.getServletContext().getAttribute(servlet.getServletContextAttributeName());
    }
}
